...
const SummaryPage = ({ length, type, packageType }) => {
  const router = useRouter();
  const HandleConfirm = async (e) => {
    e.preventDefault();
    const res = await axios.post(
      `${process.env.NEXT_PUBLIC_ORIGIN_URL}/api/est2`,
      { length, type, packageType }
    );
  ...
  const findEstimation = () => {
    const lengthPrice = estData.find((item) => item.entity === "length").price;
    const typePrice = estData.find((item) => item.entity === type).price;
    const packagePrice = estData.find(
      (item) => item.entity === packageType
    ).wardrobe_price;
    const est = length * lengthPrice + typePrice + packagePrice;
    return est;
  };
