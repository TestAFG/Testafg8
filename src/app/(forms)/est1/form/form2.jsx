...
function Form2() {
  ...
  const PageDisplay = () => {
    if (page === 0) {
      return <ShapeForm formData={formData} setFormData={setFormData} />;
    } else if (page === 1) {
      return <SizeForm formData={formData} setFormData={setFormData} />;
    } else {
  ...
  if(showSummary){
    return(
      ...
      <SummaryPage length={formData.length} packageType={formData.package} shape={formData.shape} width={formData.width}/>
      </div>
    )
  }
  return (
